...
import { configureStore,createAsync,createAsyncThunk,createSlice } from "@reduxjs/toolkit";
...
array.forEach((movie)=>{
    ...
    movie.genre_ids.forEach((genre) =>{
        const name = genres.find(({id}) =>{
            if(name) movieGenres.push(name.name);
        });
        if(movie.backdrop_path){
            moviesArray.push({
                id : movie.id,
                name: movie?.orginal_name ? movie.origial_name : movie.original_title,
